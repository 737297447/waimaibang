...
import com.lingdian.waimaibang.api.IStrategy;
...
public class ErrorHintView extends RelativeLayout {
	 ...
	 * 显示无数据
	 */
	public void noData(String notext,int noImg) {
		this.notext = notext;
		this.noImg = noImg;
	...
	class NoDataError implements IStrategy {
		...
		public void operate() {
			if (noData == null) {
				...
				TextView textView = (TextView) noData.findViewById(R.id.nodata_text);
				ImageView imageview = (ImageView) noData.findViewById(R.id.nodata_img);
				if(!notext.equals("")){
					textView.setText(notext);
				}
				if(noImg != 0){
					imageview.setImageResource(noImg);
				}
